...
class Parse:
    ...
    def exportCSV(self):
        ...
        with open('/home/output/courses.csv', 'w', encoding='UTF8', newline='') as courses:
    ...
    def getEmails(self):
        emails = set()
        regexStr = "mailto:(?:[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*|\"(?:[" \
        ...
        for email in self.soup.find_all("a", {"href": re.compile(regexStr)}):
            email = str(email).split("href=\"mailto:")[1]
            email = email.split("\"")[0]
            emails.add(email)
        return emails
